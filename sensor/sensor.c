...
void network_handler(Address *src, uint8_t* payload, uint8_t len) {
    ...
    PRINTF("SENS>> Received packet from Thing: %s\n", net_ntoa(src));
    PRINTF("SENS>> Received a %s command\n", cmdnames[cmd]);
    PRINTF("SENS>> Message for channel %d\n", dp->hdr.dst_chan_num);
    

    switch(cmd) { /* Drop packets for cmds we don't accept */
        case(QUERY): iot_query_handler(&home_chan, dp, src); return;
        case(CONNECT): iot_connect_handler(ctable_new_channel(), dp, src); return;
        case(QACK):
        case(RESPONSE):
        case(RSYN):
        case(DACK): PRINTF("SENS>> Received a silly command\n"); return;
    }
    ...
    switch(cmd) {
        case(CACK): iot_sensor_cack_handler(state, dp); break;
        case(RACK): iot_rack_handler(state, dp); break;
