...
void iot_connect(ChanState *state, uint8_t addr, int rate){
	ConnectMsg *cm;
	DataPayload *new_dp;
...
uint8_t iot_controller_cack_handler(ChanState *state, DataPayload *dp){
	...
	PRINTF("iot>> %d accepts connection request on channel %d\n", 
		state->remote_addr,
...
event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) {
	return signal iot_receive(AMPacket.source(msg), msg, payload, len);
}
